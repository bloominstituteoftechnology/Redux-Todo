...
class List extends Component {
	...
	addItem = (e) => {
		...
		// 	cursor: 'pointer'
		// };
		test.color[this.props.todos.length] = 'red'
	};
	completeI = (i, item) => {
		// for (let j = i; i >= j; j++) {
		if (test.color[i] === 'blue') {
			// test = {
			// 	color: '',
			// 	color: 'red',
			// 	cursor: 'pointer'
					...
					{this.props.todos.map((item, i) => {
						...
						return (
							<div style={lstyle} key={item + i}>
								...
								<div>
									<button style={bstyle} onClick={() => this.removeItem(item)}>
										Remove Todo
									</button>
								</div>
								{/* <div> */}

								<h3 style={cstyle} onClick={() => this.completeI(i, item)}>
