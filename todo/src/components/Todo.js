  ...
  return (
   <div className="App">
    ...
    <p className="App-intro">
     ...
     <button onClick={this.deleteCompleted}>Delete Complete</button>
    ...
    <div>
     {this.props.list.map((item, i) => {
      return (
       <div key={i}>
        <div style={item.completed ? { textDecoration: 'line-through' } : null}>
         {' '}
         {item.value}
         <button onClick={() => this.toggleComplete(item.id)}>
          {item.completed ? this.state.clickedText : this.state.text}
         </button>
...
export default connect(
 mapStateToProps,
 { addTodo, toggleComplete, deleteTodo, deleteCompleted, deleteAll }
)(Todo);
